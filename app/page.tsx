...
export default async function HomePage() {
  ...
  const { items: books, source: bookSource } = await fetchRecentBooks();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      ...
      <main className="mx-auto max-w-5xl space-y-16 px-6 pb-24 pt-12">
        ...
        <section className="grid gap-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-6" id="books">
          <div className="space-y-2">
            <p className="text-sm uppercase tracking-[0.2em] text-emerald-300">Data</p>
            <h2 className="text-2xl font-semibold text-white">최근 책 목록</h2>
            <p className="text-sm text-slate-300">
              {bookSource === 'supabase'
                ? 'Supabase 실데이터를 서버 컴포넌트에서 바로 페칭합니다.'
                : '환경 변수가 없는 경우 안전한 샘플 데이터를 SSR로 렌더링합니다.'}
            </p>
          </div>

          <div className="grid gap-4 md:grid-cols-2">
            {books.map((book) => (
              <article key={book.id} className="space-y-2 rounded-xl border border-slate-800 bg-slate-950/80 p-4 shadow-lg shadow-emerald-500/5">
                <div className="flex items-center justify-between text-sm text-slate-400">
                  <span>{book.author}</span>
                  <span>{new Date(book.publishedAt).toLocaleDateString()}</span>
                </div>
                <h3 className="text-lg font-semibold text-white">{book.title}</h3>
                {book.summary ? <p className="text-sm text-slate-300">{book.summary}</p> : null}
                <p className="text-xs text-slate-500">게시자: {book.userId.slice(0, 8)}</p>
              </article>
            ))}
          </div>
        </section>
